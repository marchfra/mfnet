...
type XFactory = Callable[[tuple[int, int]], Tensor]


@pytest.fixture
def x_factory(rng: Generator) -> XFactory:
    def create_x(shape: tuple[int, int]) -> Tensor:
        _x = rng.standard_normal(shape)
...
def test_linear_init_weights_shape() -> None:
    ...
    assert layer.weights.shape == (out_features + 1, in_features)
...
def test_init_weights_shape_and_bias_row() -> None:
    ...
    # Should have shape (out_features + 1, in_features)
    assert weights.shape == (shape[0] + 1, shape[1])
    # First row should be [1, 0, 0, 0]
    np.testing.assert_array_equal(weights[0], tensor([1, 0, 0, 0]))


def test_init_weights_raises_on_empty_shape() -> None:
    with pytest.raises(
        ValueError,
        match="Weights must be initialized with a non-empty shape.",
    ):
        Linear._init_weights((0, 0))
...
def test_linear_forward_output_shape(x_factory: XFactory) -> None:
    ...
    x = x_factory((in_features - 1, batch_size))
    out = layer.forward(x)
...
def test_linear_forward_stores_inputs(x_factory: XFactory) -> None:
    ...
    x = x_factory((in_features - 1, batch_size))
    layer.forward(x)
    assert layer.inputs.shape == (in_features, batch_size)
...
def test_linear_backward_output_shape(x_factory: XFactory) -> None:
    ...
    assert out.shape == (in_features, batch_size)
